...
import com.tieuvy.android.base_mvvm_android_2.data.storage.network.api_client.AppClient.mapError
...
class UserRepositoryImpl(private val remote: UserDataSource.Remote, private val local: UserDataSource.Local): UserRepository,BaseRepository() {

    override suspend fun searchUserByName(name: String): Flow<DataState<BaseResponseGitHub<User>>> = flow {
    ...
    override suspend fun searchUserByNameLocal(name: String): Flow<DataState<BaseResponseGitHub<User>>>  = flow {
        emit(DataState.Loading)
    ...
    override suspend fun saveSearchResult(searchResult: SearchCacheLocal): Flow<DataState<Long>> = flow {
        emit(DataState.Loading)
        val result = getResult(transformError = ::mapError) { local.saveSearchResult(searchResult) }
        emit(result)
    }.flowOn(Dispatchers.IO).catch {
        emit(DataState.Error(Exception(it.message)))
    }
