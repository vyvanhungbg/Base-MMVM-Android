...
class HomeViewModel(
    ...
    private fun saveSearchResultOnline(keySearch: String, data: BaseResponseGitHub<User>?) {
        executeTaskFlow(
            ...
            onSuccess = {
                Timber.e("saveSearchResultOnline: success ${it}")
            },
            onError = {
                Timber.e("saveSearchResultOnline: ${it.message}")
    ...
    private fun searchUsersByNameOnline(name: String) {
        executeTaskFlow(
            ...
            onError = {
                it.printStackTrace()
                Timber.e(TAG, "searchUsersByName: ${it.message}")
    ...
    private fun searchUsersByNameLocal(name: String) {
        executeTaskFlow(
            ...
            onError = {
                it.printStackTrace()
                Timber.e(TAG, "searchUsersByNameLocal: ${it.message}")
                _users.value = emptyList()
            }
        )
    }
